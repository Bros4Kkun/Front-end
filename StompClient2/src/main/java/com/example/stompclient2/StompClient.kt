...
class StompClient(private val okHttpClient: OkHttpClient,
    ...
    override fun onMessage(webSocket: WebSocket, text: String) {
        ...
        try{
            if(result.contains("/topic/chatroom/")){
                msg = handler.obtainMessage(2)
                handler.handleMessage(msg)}
            else if(result.contains("/queue/orderer/")){
                msg = handler.obtainMessage(1)
                handler.handleMessage(msg)
            }
            else if(result.contains("/topic/match/chatroom/")){
                msg = handler.obtainMessage(3)
                handler.handleMessage(msg)
            }
            else if(result.contains("/topic/match/request/")){
                msg = handler.obtainMessage(4)
                handler.handleMessage(msg)
            }
            else if(result.contains("/topic/match/done/")){
    ...
    private fun handleMessage(message: Message):String {
       ...
       response = message.headers.toString()
        return response
    }
